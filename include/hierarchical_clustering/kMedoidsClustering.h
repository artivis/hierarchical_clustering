...
#ifndef HIERARCHICAL_CLUSTERING_KMEDOIDSCLUSTRING_H
#define HIERARCHICAL_CLUSTERING_KMEDOIDSCLUSTRING_H
...
#include "hierarchical_clustering/featureAllocator.h"
...
namespace clustering
{
  /**
    ...
    *
    * Following the Partitioning Around Medoids strategy
    *
    */
  template <typename Feature,
            typename Distance = distance::EuclideanDistance<Feature, KahanAccumulator<double> >,
            typename SeedInitializer = initializer::RandomSeedsInitialization<Feature>,
  ...
  public:
    ...
    /**
      ...
      *
      */
    KMedoids(uint clusters = 0, uint maxIterations = 100, double minError = 0.001);
  ...
  KMedoids<Feature, Distance, SeedInitializer, Accu>::KMedoids(uint clusters, uint maxIterations, double minError) :
      _k(clusters),
  ...
  template <typename Feature, typename Distance, typename SeedInitializer, typename Accu>
  bool KMedoids<Feature, Distance, SeedInitializer, Accu>::initializeSeeds(const std::vector<Feature>& points,
  ...
  template <typename Feature, typename Distance, typename SeedInitializer, typename Accu>
  double KMedoids<Feature, Distance, SeedInitializer, Accu>::clusterPoints(const std::vector<Feature>& points,
                                                                         ...
                                                                         bool init_seeds)
  {
    ...
    for (uint i = 0; i < _maxIterations; ++i)
    {
      ...
      uint c;
      std::vector<double> dist_sum(this->_k, 0.);

      // Sum distances (per cluster) from each object to its cluster medoid
      #pragma omp parallel for shared(c, dist_sum) collapse(2)
      for (c = 0; c < this->_k; ++c) // foreach medoid
      {
        // Number of assigned features
        for (size_t a = 0; a < assignment_ptr[c].size(); ++a)
          dist_sum[c] += this->_distance(seeds[a], (*(assignment_ptr[c][a])));
      }
      ...
      for (uint c = 0; c < this->_k; ++c) // foreach medoid
      {
...
//   TODO typedef default e.g. EuclideanKMedoids

} // namespace clustering

#endif //HIERARCHICAL_CLUSTERING_KMEDOIDSCLUSTRING_H
