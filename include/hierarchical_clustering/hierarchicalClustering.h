...
template <typename Feature,
      ...
      public KMeans<Feature, Distance, SeedInitializer, Accu>
  {
    ...
    /**
      * Default constructor. It set the number of clusters and levels to zero, they have to be set later on.
      ...
      *
      */
    HierarchicalClustering(uint clusters = 0, uint levels = 0, uint maxIterations = 100, double minError = 0.001);
     ...
     *
     */
    void hierarchicalClusterPoints(const std::vector<Feature>& points, std::vector<Feature>& seeds)
    {
      if (!_assertParams())
      {
      ...
      for (uint level = 0; level < _levels; ++level)
      {
        ...
        for (size_t is = 0; is < ie; ++is)
        {
          ...
          std::vector<Feature> local_seeds(seeds_num, Feature(feature_size, 0.));
      ...
      seeds.resize( _comp_leafs_count );
      //_centroids.resize( _comp_centroids_count );
    }
     ...
     *
     */
    void hierarchicalClusterPoints(const std::vector<Feature>& points, std::vector<Feature>& seeds, std::vector<uint>& assignment)
    {
    ...
    std::vector<Feature> _centroids;

    /**< The number of level. */
    uint _levels;

    bool virtual _assertParams()
      {return (this->_k > 0) && (this->_levels >= 0) &&
              (this->_maxIterations > 0) && (this->_minError > 0);}
  ...
  private:
    ...
    bool _reserve()
    {
      ...
      //_centroids.reserve( _exp_centroids_count );
    }
