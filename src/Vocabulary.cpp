...
#include <hierarchical_clustering/Vocabulary.h>
#include <hierarchical_clustering/KMeansClustering.h>

typedef std::vector<double> feat;

HistogramFeatureWord::HistogramFeatureWord(const std::vector<double>& histogram,
                                           const HistogramDistance<double>* distance,
    ...
    m_weights(weights)
{
    ...
    kmeans::KMeansClustering<feat> km(3,5);
}
...
{
  if(!other || m_mean.size() != other->m_mean.size())
    return;
